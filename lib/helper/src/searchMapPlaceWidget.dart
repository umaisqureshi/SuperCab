...
import 'package:supercab/helper/src/placeType.dart';
import 'package:supercab/utils/settings.dart';
import 'package:supercab/helper/src/place.dart';
import 'dart:convert';
...
class _SearchMapPlaceWidgetState extends State<SearchMapPlaceWidget>
  ...
  void initState() {
    ...
    mobileLanguage.addListener(() {

      if(_textEditingController.text !=null && mounted) {
        getUserCurrentAddress(
            _textEditingController.text.toString(), API_KEY, mobileLanguage.value.languageCode).then((value) {
          setState(() {
            _textEditingController.text= value;
            print("***********controller: ${_textEditingController.text}");
          });
        });
      }
    });
  ...
  Future<String> getUserCurrentAddress(location, apiKey, String lang) async {
    return await changeAddressWRTLang(location, apiKey, lang);
  }

  Future<String> changeAddressWRTLang(address, String apiKey, String lang) async {
    try {
      var endPoint =
          'https://maps.googleapis.com/maps/api/geocode/json?address=$address&language=$lang&key=$apiKey';
      ...
      final String _address = response['results'][0]['formatted_address'];
      print(_address * 2);
      return _address;
    } catch (e) {
      print(e);
